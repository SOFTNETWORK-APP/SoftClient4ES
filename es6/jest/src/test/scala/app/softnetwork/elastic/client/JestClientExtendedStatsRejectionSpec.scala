...
import app.softnetwork.elastic.client.jest.{JestClientApi, JestSearchBodySerializer}
...
/** Issue #222 (story BIDC-3) -- the ES 6 Jest-client half: `STDDEV` / `VARIANCE` (the whole
  ...
  */
class JestClientExtendedStatsRejectionSpec extends AnyWordSpec with Matchers {
  ...
  private def assertRefused(refusal: ElasticError): Unit = {
    refusal.message shouldBe JestSearchBodySerializer.TransformExtendedStatsUnsupported
  ...
  s"the ES $expectedMajor client" should {
    ...
    "inject the refusing serializer" in {
      client.searchBodySerializer shouldBe JestSearchBodySerializer
      JestSearchBodySerializer.ElasticsearchMajor shouldBe expectedMajor
    }
  }
}
