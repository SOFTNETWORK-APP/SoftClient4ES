...
trait RestHighLevelClientSearchApi extends SearchApi with RestHighLevelClientHelpers {
  ...
  /** Issue #222 -- the search-body serializer every `SingleSearch` conversion made from this trait
    ...
    * JSON exists -- never executed against the raw field. See
  ...
  /** `/{indices}/{types}/_search` — empty segments are skipped and each segment is percent-encoded,
    * mirroring the high-level client's endpoint building (mapping types are still first-class on
    * 6.x). Commas and wildcards stay raw; date-math characters are encoded.
    */
...
trait RestHighLevelClientScrollApi extends ScrollApi with RestHighLevelClientHelpers {
  ...
  /** Extract ONLY hits (for search_after optimization)
    ...
    * [[extractAllResults]]. ES 6 has no PIT, so this is the only deep-paging alternative to the
    * classic scroll on this client.
    */
