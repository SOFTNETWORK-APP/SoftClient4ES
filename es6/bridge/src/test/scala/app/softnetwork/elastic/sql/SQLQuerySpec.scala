...
class SQLQuerySpec extends AnyFlatSpec with Matchers {
  ...
  it should "handle date_add function as script field" in {
    ...
    """{
      ...
      |      "script": {
      |        "lang": "painless",
      |        "source": "def param1 = (doc['createdDate'].size() == 0 ? null : doc['createdDate'].value.plus(1, ChronoUnit.WEEKS)); param1"
      |      }
      |    }
      |  },
      |  "_source": {
  ...
  it should "handle date_sub function as script field" in { // 30
    ...
    """{
      ...
      |      "script": {
      |        "lang": "painless",
      |        "source": "def param1 = (doc['createdDate'].size() == 0 ? null : doc['createdDate'].value.minus(2, ChronoUnit.MONTHS)); param1"
      |      }
      |    }
      |  },
      |  "_source": {
  ...
  /** ⚠️ Issue #367 moved this expectation: a safe cast's `try`/`catch` is now HOISTED into the
    * script prologue so it can also be a predicate operand, where a statement is a compile error.
    * Same semantics — a failed safe cast is NULL. This es6 copy is HAND-MAINTAINED (the es7/8/9
    * copies are generated from `bridge/`), so it must be edited alongside the template.
    */
  ...
  it should "handle case function as script field" in { // 40
    ...
    """{
      ...
      |      "script": {
      |        "lang": "painless",
      |        "source": "def param1 = (doc['lastUpdated'].size() == 0 ? null : doc['lastUpdated'].value); def param2 = ZonedDateTime.ofInstant(Instant.ofEpochMilli(params.__now__), ZoneId.of('Z')).minus(7, ChronoUnit.DAYS); def param3 = param1 == null ? false : (param1.isAfter(param2)); def param4 = (doc['lastSeen'].size() == 0 ? null : doc['lastSeen'].value); def param5 = (doc['lastSeen'].size() == 0 ? null : doc['lastSeen'].value.toInstant().atZone(ZoneId.of('Z')).plus(2, ChronoUnit.DAYS)); def param6 = (doc['createdAt'].size() == 0 ? null : doc['createdAt'].value); param3 ? param1 : param4 != null ? param5 : param6",
      |        "params": {
      |          "__now__": 1767139200000
      |        }
      |      }
