    ...
    stmt match {
      case ct @ CreateTable(
        ...
        cols
          ...
          // ⚠️ `cols` is the RAW parsed column list, not `ct.schema.columns`: the schema is
          // attached by `Table.update()` when the schema is built (story 21.8 Part G), so this
          // pin is the UNRESOLVED processor and its operand is still the raw `ctx.birthdate`.
          // The resolved form -- the one `CREATE TABLE` actually deploys -- is asserted below.
          //
          // What DID move here is the ingest clock. The old emission read the timestamp from
          // `ctx` under the `_ingest` key, and that is NULL on ES 6.8, 7.17, 8.18 AND 9.0 --
          // verified on real indices, not `_simulate` -- so `CURRENT_DATE` inside `SCRIPT AS` has
          // never worked on any supported version and this column was silently absent.
          // `System.currentTimeMillis()` is whitelisted on all four and is already the unit the
          // surrounding `Instant.ofEpochMilli(...)` expects. `CURRENT_DATE` also stops narrowing
          // to a `LocalDate` in a processor, so both sides of `between` are the same type.
          """def param1 = ctx.birthdate;
            |def param2 = ZonedDateTime.ofInstant(Instant.ofEpochMilli(System.currentTimeMillis()), ZoneId.of('Z'));
            |def param3 = Long.valueOf(ChronoUnit.YEARS.between(param1, param2));
            |ctx.age = (param1 == null) ? null : param3""".stripMargin.replaceAll("\n", " ")
        )
        // The RESOLVED processor, which is what `CREATE TABLE` deploys. The operand is parsed
        // first and its runtime shape decided by `instanceof`, because `ctx.birthdate` is the raw
        // JSON value of the document being indexed. This exact pipeline was run on all four
        // majors: {"birthdate":"1990-05-20"} and {"birthdate":643161600000} both store `age: 36`.
        ct.schema.columns
          .find(_.name == "age")
          .flatMap(_.script)
          .map(_.source)
          .getOrElse("") should include(
          """def param1 = ctx.birthdate;
            |def param2 = (param1 instanceof String ? LocalDate.parse((param1).replace("/", "-"), DateTimeFormatter.ofPattern("yyyy-MM-dd")).atStartOfDay(ZoneId.of('Z')) : Instant.ofEpochMilli(param1).atZone(ZoneId.of('Z')));
            |def param3 = ZonedDateTime.ofInstant(Instant.ofEpochMilli(System.currentTimeMillis()), ZoneId.of('Z'));
            |def param4 = Long.valueOf(ChronoUnit.YEARS.between(param2, param3));
            |ctx.age = (param1 == null) ? null : param4""".stripMargin.replaceAll("\n", " ")
        )
      ...
      case u @ Update("products", values, Some(where), _) =>
        ...
        pipeline.processors.find(_.column == "updated_at") match {
          case Some(updatedAtProc) =>
            updatedAtProc
              .asInstanceOf[ScriptProcessor]
              .source shouldBe "def param1 = ZonedDateTime.ofInstant(Instant.ofEpochMilli(System.currentTimeMillis()), ZoneId.of('Z')); ctx.updated_at = param1"
