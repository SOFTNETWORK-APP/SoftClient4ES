...
trait JavaClientIndicesApi extends IndicesApi with JavaClientHelpers {
  ...
  override private[client] def executeGetIndex(index: String): ElasticResult[Option[String]] = {
    executeJavaAction(
      ...
      val indices = response.result.asScala
...
trait JavaClientIndexApi extends IndexApi with JavaClientHelpers {
  ...
  override private[client] def executeIndexAsync(
    ...
    fromCompletableFuture(
      ...
      resp.result() match {
        case Result.Created | Result.Updated | Result.NoOp => ElasticSuccess(true)
        case _                                             => ElasticSuccess(false)
      }
    }
}
...
trait JavaClientDeleteApi extends DeleteApi with JavaClientHelpers {
  ...
  override private[client] def executeDeleteAsync(index: String, id: String, wait: Boolean)(implicit
    ...
    fromCompletableFuture(
      ...
      resp.result() match {
        case Result.Deleted | Result.NoOp => ElasticSuccess(true)
        case _                            => ElasticSuccess(false)
      }
    }
    ...
    executeJavaAction(
      ...
      resp
        ...
        .map { case (id, pipeline) =>
          val json = convertToJson(pipeline)
          id -> json
        }
        .toMap
    }
...
trait JavaClientTemplateApi extends TemplateApi with JavaClientHelpers {
  ...
  override private[client] def executeGetLegacyTemplate(
    ...
    executeJavaAction(
      ...
      resp.result().asScala.get(templateName).map { template =>
        convertToJson(template)
      }
    }
  }
