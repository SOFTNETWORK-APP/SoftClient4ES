...
-- 5 rows: each order paired with itself

SELECT a.id AS left_id, b.id AS right_id
FROM orders a
JOIN orders b ON a.customer_id = b.customer_id;
-- every pair of orders sharing a customer (fan-out, see below)
```

> **Since engine `0.23.0` with arrow-extensions `0.3.3`.** Before that, the alias map kept only the *last* alias of a repeated index, so the first leg's columns never resolved, the `ON` clause lost its join key and DuckDB answered `Parser Error: syntax error at end of input` (softclient4es-arrow#144). Two rules that follow from the fix: **qualify every column** (a bare `id` is ambiguous between the two legs and is rejected as such), and write a self-join with `JOIN … ON` — the comma form `FROM orders a, orders b` is a multi-index *search* with no join engine behind it and is now rejected with a message pointing at the `JOIN` spelling.
