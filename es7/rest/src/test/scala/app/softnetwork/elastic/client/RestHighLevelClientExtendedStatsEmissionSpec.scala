...
/** Issue #222 (story BIDC-3) -- the ES 7 half, since the elastic4s 7.17.26 migration (AD-S3-4).
  *
  * `STDDEV` / `VARIANCE` (the whole `extended_stats` family) over a TRANSFORMED expression carries
  * its script in the emitted JSON, on BOTH serialisation doors and for BOTH binds (plain, and
  * windowed under a partition bucket) -- exactly as on ES 8 / ES 9, but by a different mechanism:
  * this module has no `customAggregation` seam, so it UNWRAPS the bridge's marker and lets the
  * stock 7.17.26 builder (which now emits `agg.script`, elastic4s#4105) render it.
  *
  * This file REPLACES `RestHighLevelClientExtendedStatsRejectionSpec`: until 7.17.26 the same
  * statements were refused with a named 400, because the library dropped the script and the
  * alternative was a silently wrong statistic over the raw field.
  *
  ...
  */
class RestHighLevelClientExtendedStatsEmissionSpec extends AnyWordSpec with Matchers {
  ...
  /** The script of every marker in a tree -- what the unwrap must preserve verbatim. */
  ...
  "the ES 7 client (elastic4s 7.17.26+)" should {
    ...
    "emit a pure script when the transform has no raw field behind it (ABS(salary))" in {
      // Before 7.17.26: `"extended_stats":{}` -- neither field nor script, rejected by Elasticsearch
      // (and refused by this module rather than sent).
  ...
  "RestHighLevelClientSearchBodySerializer.unwrap" should {

    "substitute the marker for the aggregation it wraps, at every depth" in {
      Seq(plain("STDDEV", "YEAR(createdAt)"), windowed("VARIANCE", "ABS(salary)")).foreach { sql =>
        withClue(s"[$sql] ") {
          val request: ElasticSearchRequest = requestToElasticSearchRequest(single(sql))
          request.hasTransformExtendedStats shouldBe true
          val unwrapped = RestHighLevelClientSearchBodySerializer.unwrap(request.search)
          // No marker survives, and the scripts are carried over unchanged.
          SearchBodySerializer.hasTransformExtendedStats(unwrapped) shouldBe false
          markerScriptsOf(request.search.aggs) should have size 1
          mapper
            .readTree(SearchBodySerializer.Default.serialize(unwrapped))
            .findValues("script")
            .asScala
            .flatMap(s => Option(s.get("source")).map(_.asText())) shouldBe
          markerScriptsOf(request.search.aggs)
        }
      }
    }

    "leave a request without a marker untouched (byte-identical to the stock builder)" in {
      Seq(
        plain("STDDEV", "salary"),
        ...
        withClue(s"[$sql] ") {
          ...
          RestHighLevelClientSearchBodySerializer.unwrap(request.search) shouldBe request.search
          RestHighLevelClientSearchBodySerializer.serialize(request.search) shouldBe
          SearchBodySerializer.Default.serialize(request.search)
        }
      }
    }

    "keep a raw-field extended_stats exactly as it was" in {
      emitted(plain("STDDEV", "salary")) should include(""""extended_stats":{"field":"salary"}""")
    }

    "be the serializer the client injects" in {
      client.searchBodySerializer shouldBe RestHighLevelClientSearchBodySerializer
      RestHighLevelClientSearchBodySerializer.ElasticsearchMajor shouldBe 7
    }
  }
}
